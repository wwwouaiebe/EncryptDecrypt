...
(function() {
	...
	*/
	...
	var pswdDialog = function ( onOk, onCancel ) {
		var pswdMainDiv = document.getElementById ( "pswdMainDiv" );
		...
		document.getElementById ( "okButton" ).addEventListener (
			...
			function ( ) {
				...
				onOk ( new window.TextEncoder ( ).encode ( pswd ) );
			}
		);
	...
	*/
	...
	function saveDecryptedDataToFile ( decryptedData ) {
		var tmpArray = [];
		for ( var property in decryptedData.data ) {
		  tmpArray.push ( decryptedData.data [ property ] );
		}
		var blob = new Blob(
		...
		element.setAttribute( 'download', decryptedData.name );
	...
	*/
	...
	document.getElementById ( "decryptButton" ).addEventListener (
		"change",
		function ( event ) {
	...
	*/
	...
	document.getElementById ( "encryptButton" ).addEventListener (
		"change",
		function ( event ) {
			document.getElementById ( "errorDiv" ).innerHTML = "";
			...
			fileReader.onload = function ( event ) {
				// Yes, I know... It's not necessary to save to an object, but the DataEncryptor.js is mainly
				// used to save JavaScript objects to file in others projects, so I will not change...
